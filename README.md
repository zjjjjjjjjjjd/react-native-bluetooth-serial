...
## Installation
1. Install package via npm: `npm i -S react-native-bluetooth-tg`
...
## Manual installation
#### iOS
1. `npm i -S react-native-bluetooth-tg`
2. In XCode, in the project navigator, right click `Libraries` ➜ `Add Files to [your project's name]`
3. Go to `node_modules` ➜ `react-native-bluetooth-tg` and add `RCTBluetoothSerial.xcodeproj`
...
3. Append the following lines to `android/settings.gradle`:
    ```
    include ':react-native-bluetooth-tg'
    project(':react-native-bluetooth-tg').projectDir = new File(rootProject.projectDir, '../node_modules/react-native-bluetooth-tg/android')
    ```
4. Insert the following lines inside the dependencies block in `android/app/build.gradle`:
    ```
    compile project(':react-native-bluetooth-tg')
    ```
...
1. `git clone https://github.com/rusel1989/react-native-bluetooth-tg.git`
2. `cd react-native-bluetooth-tg/BluetoothSerialExample`
3. `npm i`
